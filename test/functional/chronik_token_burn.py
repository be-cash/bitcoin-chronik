...
class ChronikTokenBurn(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        bare_burn = TokenTx(
            ...
            inputs=[
                slp_token(token_id=genesis_slp.txid, is_mint_baton=True, entry_idx=1),
                alp_token(token_id=genesis_alp.txid, amount=400),
            ],
            outputs=[
                pb.Token(),
        ...
        bare_burn.send(
            chronik,
            error=f"400: Tx {bare_burn.txid} failed token checks: Unexpected burn: Burns 400 base tokens. Unexpected burn: Burns mint baton(s).",
        )
