...
be in ascending order""",
        ...
        for block_idx in reversed(range(len(txs))):
            ...
            # Bitcoin Core currently starts randomly kicking out txs on reorg
            # On Bitcoin ABC, this would work (TODO: Investigate):
            # for tx, block_hash in zip(txs, tx_block_hashes):
            #    tx.test(chronik, block_hash)
        ...
        conflict_tx.vout = [CTxOut(546, P2SH_OP_TRUE), CTxOut(546, P2SH_OP_TRUE)]
        block = create_block(
            int(block_hashes[-1], 16),
            create_coinbase(block_height, b"\x03" * 33),
            1300000500,
