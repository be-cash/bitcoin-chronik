...
class ChronikTokenScriptGroup(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        script1_txs = [genesis_slp, genesis_alp, send_slp]
        script1_txs_sorted = sorted(script1_txs, key=lambda tx: tx.txid)
        history_txs = chronik.script("p2sh", "01" * 20).unconfirmed_txs().ok().txs
        for tx, proto_tx in zip_longest(script1_txs_sorted, history_txs):
            tx.test_tx(proto_tx)
        ...
        script1_utxos_sorted = sorted(script1_utxos, key=lambda o: o.outpoint.txid[::-1])
        utxos = chronik.script("p2sh", "01" * 20).utxos().ok().utxos
        for utxo, proto_utxo in zip_longest(script1_utxos_sorted, utxos):
