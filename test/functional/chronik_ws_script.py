...
from test_framework.script import OP_EQUAL, OP_HASH160, OP_RETURN, CScript, hash160
...
class ChronikWsScriptTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        tx2.vout = [CTxOut(0, CScript([OP_RETURN, b"0" * 70]))]
        tx2.rehash()
        ...
        tx3.vout = [CTxOut(0, CScript([OP_RETURN, b"0" * 70]))]
        tx3.rehash()
        ...
        check_tx_msgs(ws1, pb.TX_CONFIRMED, [txid, tx3_conflict.hash])

        # For ws2, this only sends the CONFIRMED msgs
        check_tx_msgs(ws2, pb.TX_CONFIRMED, [txid, txid2])
