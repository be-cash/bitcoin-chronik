...
class ChronikTokenBurn(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        for tx, proto_tx in zip_longest([genesis_slp, send_alp], history_txs):
            tx.test_tx(proto_tx, block_hash)

        history_txs = chronik.token_id(genesis_alp.txid).confirmed_txs().ok().txs
        for tx, proto_tx in zip_longest([genesis_alp, send_alp], history_txs):
            tx.test_tx(proto_tx, block_hash)

        history_txs = chronik.token_id(genesis2_alp.txid).confirmed_txs().ok().txs
        for tx, proto_tx in zip_longest([genesis2_alp, send_alp], history_txs):
            tx.test_tx(proto_tx, block_hash)
