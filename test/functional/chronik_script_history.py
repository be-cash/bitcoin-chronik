    ...
    def run_test(self):
        ...
        def tx_sort_key(blocktxids):
            def sort_fn(entry):
                ...
                idx = 0
                if blocktxids is not None:
                    # blocks are ordered in block order
                    idx = blocktxids.index(txid)
                else:
                    # Mempool is ordered by txid
                    idx = txid
                if entry.get("is_coinbase", False):
                    idx = 0
                return (time_first_seen, idx)
            return sort_fn
        ...
        newblock = node.getblock(block.hash)
        newblocktxs.sort(key=tx_sort_key(newblock["tx"]), reverse=True)
